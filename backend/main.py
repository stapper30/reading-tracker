...
@app.post("/books/add")
async def add_book(bookPost: BookPydantic, current_user: User = Depends(get_current_user)):
    print(f"User {current_user.email} is adding a book: {bookPost}")
    bookRepo = BookRepo(db_pool=pool)
    book = Book(title=bookPost.title, author=bookPost.author, rating=bookPost.rating, user_id=current_user.id, type=bookPost.type, image_url=bookPost.image_url)
    await bookRepo.add_book(book)
    books = await bookRepo.get_books_for_user(current_user.id)
    return [book.to_dict() for book in books]

@app.post("/books/mark_read/{id}")
async def mark_book_as_read(id: int, rating: Annotated[int, Body()], current_user: User = Depends(get_current_user)):
    print(f"User {current_user.email} is marking book id {id} as read with rating {rating}")
    bookRepo = BookRepo(db_pool=pool)
...
async def delete_book(id: int, current_user: User = Depends(get_current_user)):
    bookRepo = BookRepo(db_pool=pool)
